document.addEventListener('DOMContentLoaded', () => {
    ...
    function openTemplateModal(id = null) {
        let template = { name: '', content: '', manualFields: [], fontFamily: 'Helvetica' };
        ...
        updatePlaceholders(template.manualFields);
    }
    ...
    function saveTemplate() {
        ...
        if (id) {
            ...
            if (index > -1) appData.templates[index] = { ...appData.templates[index], name, content, manualFields, fontFamily };
        } else {
            appData.templates.push({ id: `template_${Date.now()}`, name, content, manualFields, fontFamily });
        }
        saveData();
        renderTemplates();
    ...
    function updatePlaceholders(manualFields = []) {
        const placeholdersContainer = document.getElementById('placeholders-container');
    ...
    function generatePDF() {
        ...
        pendingPDFGeneration = { template, rowData };
    ...
    async function downloadPDF() {
        ...
        const { finalContent, template } = pendingPDFGeneration;
        ...
        if(!finalFilename) return; // User cancelled prompt
        ...
        try {
            ...
            paragraphs.forEach(paragraph => {
                ...
                segments.forEach(segment => {
                    ...
                    const lines = doc.splitTextToSize(text, usableWidth - (xPos - margin));
                    ...
                    lines.forEach((line, index) => {
                        ...
                        if (index < lines.length - 1) {
                            ...
                            xPos += doc.getStringUnitWidth(line) * fontSize / doc.internal.scaleFactor;
                        }
                    });
                });
                cursorY += lineHeight;
